...
# bash <(wget -qO- https://raw.githubusercontent.com/spiyushk/agent/master/scripts/agentInstaller.sh) server111 6 lKey101 
	...
	echo "station 4"
     ...
     echo " $command ${start}"
     echo "station 7"
     export stopCommand="( sleep 10 ) && (sh $command ${stop}) &"
     echo $stopCommand
     stopCommandOutput=$(( sleep 10 ) && (sh $command ${stop}) &)
     ...
     echo "station 9"
     echo "station 10"
     export startCommand="( sleep 15 ) && (sh $command ${start}) &"
     startCommandOutput=$(( sleep 15 ) && (sh $command ${start}) &)
